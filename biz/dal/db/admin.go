...
func AdminCreateUser(ctx context.Context, username, password string) error {
	// 检查用户名是否已存在（与 CreateUser 方法保持一致）
	var existing User
	err := DB.WithContext(ctx).Where("username = ?", username).First(&existing).Error
	if err == nil {
		return errno.NewErrNo(errno.ServiceUserExist, "用户名已存在")
	}
	if !errors.Is(err, gorm.ErrRecordNotFound) {
		return errno.NewErrNo(errno.InternalDatabaseErrorCode, "检查用户名是否存在时出错: "+err.Error())
	}
